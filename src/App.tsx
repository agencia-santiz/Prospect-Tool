...
import { Company, SearchParams, SavedList, Pipeline, Deal, PipelineStage, CardVisibilityConfig } from './types';
...
import { Search, Loader2, Hexagon, Save, LayoutGrid, List, Download, MapPin, Globe, Check, AlertOctagon, PinOff, Bell, User, PanelLeftOpen, PanelLeftClose, Filter, Plus, Phone, Globe2, MoreHorizontal, LogOut, X, RefreshCw, Settings, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedCardConfig = localStorage.getItem(CARD_CONFIG_STORAGE_KEY);
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (segmentInputRef.current && !segmentInputRef.current.contains(target)) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('nexus_pipelines', JSON.stringify(pipelines));
  }, [isAuthenticated, workspaceHydrated, pipelines]);

  useEffect(() => {
    if (!isAuthenticated || !workspaceHydrated) return;
    localStorage.setItem(CARD_CONFIG_STORAGE_KEY, serializeCardConfig(cardConfig));
  ...
  useEffect(() => {
    if (!isAuthenticated || !workspaceHydrated) return;
    localStorage.setItem('nexus_contacted_keys', JSON.stringify(Array.from(contactedKeys)));
  ...
  const fetchLeadsWithFallback = async (
    ...
    try {
      const openDataLeads = await fetchOpenDataLeads(city, segment, excludeNames, desiredQuantity);
      if (openDataLeads.length > 0) {
        return openDataLeads;
      }
    } catch (error) {
      console.warn('Open data fallback failed, trying backend enrichment.', error);
    }

    return fetchEnrichedLeads(city, segment, excludeNames, desiredQuantity);
  };

  const handleExport = async () => {
      const { exportLeadsToCsv } = await import('./services/geminiService');
      const blob = await exportLeadsToCsv(currentLeads);
      if (!blob) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
        const results = await fetchLeadsWithFallback(searchParams.city, resolvedSegment, [], desiredQuantity);
        ...
        if (results.length === 0) {
          ...
          setError(`Nenhuma empresa encontrada para "${resolvedSegment}" em "${searchParams.city}". A cobertura de dados abertos (OpenStreetMap) pode ser limitada nesta região. Configure as chaves do Google Places ou Gemini no backend para ampliar os resultados.`);
        }
  ...
  const renderListsView = () => (
      <div className="space-y-6 animate-fadeIn">
          <div className="flex justify-between items-center bg-nexus-surface p-5 rounded border border-nexus-sand shadow-subtle">
             ...
             <button onClick={handleGoToExplore} className="flex items-center gap-2 text-sm font-bold text-white bg-nexus-royal px-5 py-2.5 rounded shadow-sm hover:bg-nexus-crimsonLight transition-all">
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 bg-nexus-surface rounded-lg shadow-float border border-nexus-sand p-2 hidden group-hover:block animate-fadeIn z-50">
                            ...
                            <div className="space-y-1 mb-2">
                                <button
                                    ...
                                    className="w-full text-left px-3 py-2 text-xs font-medium text-nexus-charcoal hover:bg-nexus-sandLight rounded flex items-center gap-2"
                                >
                                ...
                                <button
                                    ...
                                    className="w-full text-left px-3 py-2 text-xs font-medium text-nexus-royal hover:bg-nexus-accent rounded flex items-center gap-2"
                                >
